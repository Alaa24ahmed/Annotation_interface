const supabase = require('../config/supabase');

// ============================================
// PILOT USERS CONFIGURATION - EDIT HERE ONLY
// 3 verifiers per language
// ============================================
const PILOT_USERS_CONFIG = [
  // Chinese - China (3 verifiers)
  { id: '742891', language: 'Chinese', country: 'China' },
  { id: '742892', language: 'Chinese', country: 'China' },
  { id: '742893', language: 'Chinese', country: 'China' },
  
  // Indonesian - Indonesia (3 verifiers)
  { id: '586234', language: 'Indonesian', country: 'Indonesia' },
  { id: '586235', language: 'Indonesian', country: 'Indonesia' },
  { id: '586236', language: 'Indonesian', country: 'Indonesia' },
  
  // Egyptian Arabic - Egypt (3 verifiers)
  { id: '193847', language: 'Egyptian Arabic', country: 'Egypt' },
  { id: '193848', language: 'Egyptian Arabic', country: 'Egypt' },
  { id: '193849', language: 'Egyptian Arabic', country: 'Egypt' },
  
  // Hindi - India (3 verifiers)
  { id: '415672', language: 'Hindi', country: 'India' },
  { id: '415673', language: 'Hindi', country: 'India' },
  { id: '415674', language: 'Hindi', country: 'India' },
  
  // Tagalog - Philippines (3 verifiers)
  { id: '829456', language: 'Tagalog', country: 'Philippines' },
  { id: '829457', language: 'Tagalog', country: 'Philippines' },
  { id: '829458', language: 'Tagalog', country: 'Philippines' },
  
  // Amharic - Ethiopia (3 verifiers)
  { id: '129431', language: 'Amharic', country: 'Ethiopia' },
  { id: '129432', language: 'Amharic', country: 'Ethiopia' },
  { id: '129433', language: 'Amharic', country: 'Ethiopia' },
  
  // Tunisian Arabic - Tunisia (3 verifiers)
  { id: '548792', language: 'Tunisian Arabic', country: 'Tunisia' },
  { id: '548793', language: 'Tunisian Arabic', country: 'Tunisia' },
  { id: '548794', language: 'Tunisian Arabic', country: 'Tunisia' },
  
  // Brazilian Portuguese - Brazil (3 verifiers)
  { id: '305864', language: 'Brazilian Portuguese', country: 'Brazil' },
  { id: '305865', language: 'Brazilian Portuguese', country: 'Brazil' },
  { id: '305866', language: 'Brazilian Portuguese', country: 'Brazil' },
  
  // Mexican Spanish - Mexico (3 verifiers)
  { id: '762549', language: 'Mexican Spanish', country: 'Mexico' },
  { id: '762550', language: 'Mexican Spanish', country: 'Mexico' },
  { id: '762551', language: 'Mexican Spanish', country: 'Mexico' },
  
  // Zulu - South Africa (3 verifiers)
  { id: '438276', language: 'Zulu', country: 'South Africa' },
  { id: '438277', language: 'Zulu', country: 'South Africa' },
  { id: '438278', language: 'Zulu', country: 'South Africa' },
  
  // Greek - Greece (3 verifiers)
  { id: '591823', language: 'Greek', country: 'Greece' },
  { id: '591824', language: 'Greek', country: 'Greece' },
  { id: '591825', language: 'Greek', country: 'Greece' },
  
  // Kyrgyz - Kyrgyzstan (3 verifiers)
  { id: '674195', language: 'Kyrgyz', country: 'Kyrgyzstan' },
  { id: '674196', language: 'Kyrgyz', country: 'Kyrgyzstan' },
  { id: '674197', language: 'Kyrgyz', country: 'Kyrgyzstan' },
  
  // Italian - Italy (3 verifiers)
  { id: '283947', language: 'Italian', country: 'Italy' },
  { id: '283948', language: 'Italian', country: 'Italy' },
  { id: '283949', language: 'Italian', country: 'Italy' },
  
  // Turkish - Turkey (3 verifiers)
  { id: '937285', language: 'Turkish', country: 'Turkey' },
  { id: '937286', language: 'Turkish', country: 'Turkey' },
  { id: '937287', language: 'Turkish', country: 'Turkey' },
  
  // Georgian - Georgia (3 verifiers)
  { id: '482693', language: 'Georgian', country: 'Georgia' },
  { id: '482694', language: 'Georgian', country: 'Georgia' },
  { id: '482695', language: 'Georgian', country: 'Georgia' },
  
  // Thai - Thailand (3 verifiers)
  { id: '756148', language: 'Thai', country: 'Thailand' },
  { id: '756149', language: 'Thai', country: 'Thailand' },
  { id: '756150', language: 'Thai', country: 'Thailand' },
  
  // Batak Karo - Indonesia (3 verifiers)
  { id: '814527', language: 'Batak Karo', country: 'Indonesia' },
  { id: '814528', language: 'Batak Karo', country: 'Indonesia' },
  { id: '814529', language: 'Batak Karo', country: 'Indonesia' },
  
  // Yemeni Arabic - Yemen (3 verifiers)
  { id: '925371', language: 'Yemeni Arabic', country: 'Yemen' },
  { id: '925372', language: 'Yemeni Arabic', country: 'Yemen' },
  { id: '925373', language: 'Yemeni Arabic', country: 'Yemen' },
  
  // Yoruba - Nigeria (3 verifiers)
  { id: '634892', language: 'Yoruba', country: 'Nigeria' },
  { id: '634893', language: 'Yoruba', country: 'Nigeria' },
  { id: '634894', language: 'Yoruba', country: 'Nigeria' },
  
  // Moroccan Arabic - Morocco (3 verifiers)
  { id: '471836', language: 'Moroccan Arabic', country: 'Morocco' },
  { id: '471837', language: 'Moroccan Arabic', country: 'Morocco' },
  { id: '471838', language: 'Moroccan Arabic', country: 'Morocco' },
  
  // Japanese - Japan (3 verifiers)
  { id: '592847', language: 'Japanese', country: 'Japan' },
  { id: '592848', language: 'Japanese', country: 'Japan' },
  { id: '592849', language: 'Japanese', country: 'Japan' }
];

// Derived values - DO NOT EDIT
const validPilotIds = PILOT_USERS_CONFIG.map(u => u.id);
// ============================================

/**
 * Get annotations that need verification for a specific language and country
 */
async function getAnnotationsForVerification(req, res) {
  try {
    const { language, country } = req.query;

    // Validate parameters
    if (!language || !country) {
      return res.status(400).json({
        error: 'Missing required parameters: language and country'
      });
    }

    // Query annotations from the annotations_verified table
    // Only get annotations that have all required fields filled
    const { data, error } = await supabase
      .from('annotations_verified')
      .select('*')
      .eq('language', language)
      .eq('country', country)
      .not('generated_question', 'is', null)
      .not('generated_option1', 'is', null)
      .not('generated_option2', 'is', null)
      .not('generated_option3', 'is', null)
      .not('generated_option4', 'is', null)
      .not('translated_question', 'is', null)
      .order('created_at', { ascending: true });

    if (error) {
      console.error('Error fetching annotations:', error);
      return res.status(500).json({
        error: 'Failed to fetch annotations',
        details: error.message
      });
    }

    return res.status(200).json({
      annotations: data || [],
      count: data ? data.length : 0
    });

  } catch (error) {
    console.error('Error in getAnnotationsForVerification:', error);
    return res.status(500).json({
      error: 'Internal server error',
      details: error.message
    });
  }
}

/**
 * Get list of annotation IDs already verified by this verifier
 */
async function getCompletedVerifications(req, res) {
  try {
    const { verifier_id } = req.query;

    if (!verifier_id) {
      return res.status(400).json({
        error: 'Missing required parameter: verifier_id'
      });
    }

    // Query verifications table to get annotation IDs already verified
    const { data, error } = await supabase
      .from('verifications')
      .select('annotation_id')
      .eq('verifier_user_id', verifier_id);

    if (error) {
      console.error('Error fetching completed verifications:', error);
      return res.status(500).json({
        error: 'Failed to fetch completed verifications',
        details: error.message
      });
    }

    const verifiedAnnotationIds = data ? data.map(v => String(v.annotation_id)) : [];

    return res.status(200).json({
      verified_annotation_ids: verifiedAnnotationIds,
      count: verifiedAnnotationIds.length
    });

  } catch (error) {
    console.error('Error in getCompletedVerifications:', error);
    return res.status(500).json({
      error: 'Internal server error',
      details: error.message
    });
  }
}

/**
 * Save a verification result
 */
async function saveVerification(req, res) {
  try {
    const {
      annotation_id,
      template_id,
      verifier_user_id,
      selected_option,
      selected_option_text,
      is_correct,
      original_correct_position,
      shuffled_correct_position,
      subset_id,
      language,
      country,
      time_spent_seconds,
      comment
    } = req.body;

    console.log('Verification request received:', {
      annotation_id,
      template_id,
      verifier_user_id,
      selected_option,
      selected_option_text,
      is_correct,
      subset_id
    });

    // Validate required fields
    if (!annotation_id || !template_id || !verifier_user_id || !selected_option) {
      return res.status(400).json({
        error: 'Missing required fields',
        required: ['annotation_id', 'template_id', 'verifier_user_id', 'selected_option']
      });
    }

    // Validate verifier_user_id
    if (!validPilotIds.includes(verifier_user_id)) {
      return res.status(403).json({
        error: 'Invalid verifier user ID'
      });
    }

    // Validate subset_id if provided (optional since we're loading from CSV)
    if (subset_id && (subset_id < 1 || subset_id > 10)) {
      return res.status(400).json({
        error: 'Invalid subset ID. Must be between 1 and 10'
      });
    }

    // Validate selected_option format (should be A, B, C, or D)
    if (!['A', 'B', 'C', 'D'].includes(selected_option)) {
      return res.status(400).json({
        error: 'Invalid selected_option. Must be A, B, C, or D'
      });
    }

    // Check if this annotation has already been verified by this user
    const { data: existingVerification, error: checkError } = await supabase
      .from('verifications')
      .select('id')
      .eq('annotation_id', annotation_id)
      .eq('verifier_user_id', verifier_user_id)
      .single();

    if (checkError && checkError.code !== 'PGRST116') { // PGRST116 = no rows returned
      console.error('Error checking existing verification:', checkError);
      return res.status(500).json({
        error: 'Failed to check existing verification',
        details: checkError.message
      });
    }

    if (existingVerification) {
      return res.status(409).json({
        error: 'This annotation has already been verified by this user'
      });
    }

    // Prepare verification data
    const verificationData = {
      annotation_id,
      template_id,
      verifier_user_id,
      selected_option,
      selected_option_text: selected_option_text || null,
      is_correct: is_correct === true,
      original_correct_position: original_correct_position || 'A',
      shuffled_correct_position: shuffled_correct_position || selected_option,
      subset_id: subset_id || null,
      language: language || null,
      country: country || null,
      time_spent_seconds: time_spent_seconds || 0,
      comment: comment || null,
      created_at: new Date().toISOString()
    };

    // Insert verification into database
    const { data: insertedData, error: insertError } = await supabase
      .from('verifications')
      .insert([verificationData])
      .select();

    if (insertError) {
      console.error('Error inserting verification:', insertError);
      return res.status(500).json({
        error: 'Failed to save verification',
        details: insertError.message
      });
    }

    console.log('Verification saved successfully:', {
      id: insertedData[0].id,
      annotation_id,
      verifier_user_id,
      is_correct,
      subset_id: subset_id || 'N/A'
    });

    return res.status(201).json({
      success: true,
      message: 'Verification saved successfully',
      verification: insertedData[0]
    });

  } catch (error) {
    console.error('Error in saveVerification:', error);
    return res.status(500).json({
      error: 'Internal server error',
      details: error.message
    });
  }
}

/**
 * Get verification statistics (optional - for analytics)
 */
async function getVerificationStats(req, res) {
  try {
    const { verifier_id, subset } = req.query;

    let query = supabase.from('verifications').select('*');

    if (verifier_id) {
      query = query.eq('verifier_user_id', verifier_id);
    }

    if (subset) {
      const subsetId = parseInt(subset);
      if (!isNaN(subsetId)) {
        query = query.eq('subset_id', subsetId);
      }
    }

    const { data, error } = await query;

    if (error) {
      console.error('Error fetching verification stats:', error);
      return res.status(500).json({
        error: 'Failed to fetch verification statistics',
        details: error.message
      });
    }

    const stats = {
      total_verifications: data.length,
      correct_verifications: data.filter(v => v.is_correct).length,
      incorrect_verifications: data.filter(v => !v.is_correct).length,
      accuracy: data.length > 0 
        ? (data.filter(v => v.is_correct).length / data.length * 100).toFixed(2) + '%'
        : '0%',
      average_time_seconds: data.length > 0
        ? (data.reduce((sum, v) => sum + (v.time_spent_seconds || 0), 0) / data.length).toFixed(2)
        : '0'
    };

    return res.status(200).json({
      stats,
      verifications: data
    });

  } catch (error) {
    console.error('Error in getVerificationStats:', error);
    return res.status(500).json({
      error: 'Internal server error',
      details: error.message
    });
  }
}

module.exports = {
  getAnnotationsForVerification,
  getCompletedVerifications,
  saveVerification,
  getVerificationStats
};
